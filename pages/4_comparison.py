...
from utils.config import COL_NAME, COL_RANK, COL_SCORE, COL_PRIORITY, COL_AGING, COL_SINGLE_ELDERLY, COL_MEDICAL, COL_TRANSPORT
...
INDICATOR_LABELS = ["高齢化率", "単身高齢者割合", "医療アクセス", "公共交通"]
...
with tab1:
    ...
    st.markdown("#### 棒グラフ比較")
    bar_values = {label: [target_row[col], city_avg[col]] for label, col in zip(INDICATOR_LABELS, INDICATOR_COLS)}
    st.plotly_chart(
...
with tab2:
    ...
    if len(selected) < 2:
        ...
        st.markdown("#### 棒グラフ比較")
        bar_values = {label: [row[col] for row in rows] for label, col in zip(INDICATOR_LABELS, INDICATOR_COLS)}
